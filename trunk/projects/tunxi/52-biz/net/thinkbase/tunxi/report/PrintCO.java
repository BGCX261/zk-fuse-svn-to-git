...
public class PrintCO implements ReportRunner {
	...
	public JasperPrint fill(HttpServletRequest req, UserAccount user) throws Exception {
		...
		String sql = TextFileUtil.readAsText(
				this.getClass().getResourceAsStream("PrintCO.sql"));
		...
		if ("A4".equalsIgnoreCase(req.getParameter("P"))){	//A4 纸打印
			in = this.getClass().getResourceAsStream("CO_A4.jasper");
		}else{
			in = this.getClass().getResourceAsStream("CO.jasper");
		}
